...
def fit_plot_parabola(xdata,ydata,yuncert,d,xflag):
    ...
    # print(yuncert)
    coef, cov = curve_fit(inst.fitfunc,xdata,ydata)
    A = coef[0]
    B = coef[1]
    C = coef[2]
...
def new_quad_scan925(foc,sigsqr,uncert):
    ...
    plt.errorbar(foc,sigsqr,uncert,marker='o',capsize=4,color='green',label='Data',linestyle='')

    xth = np.linspace(np.min(foc),np.max(foc),200)
    fitted_data = parab(xth,A,B,C)
    plt.plot(xth,fitted_data,color='red',label='Parabola fit')
    ...
    return (alpha,beta,gamma,eps,0,0,0,0)
